...
export const initializeSocket = (httpServer: HTTPServer) => {
  ...
  io.use(async (socket: AuthenticatedSocket, next) => {
    try {
      ...
      console.log("Socket Handshake Cookie:", rawCookie);

      if (!rawCookie) return next(new Error("Unauthorized: No cookie"));
      ...
      if (!decodedToken) return next(new Error("Unauthorized: Invalid token"));
